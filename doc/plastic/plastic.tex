...
\usepackage{color}
\usepackage{graphicx}
\PassOptionsToPackage{hyphens}{url}\usepackage[pdftitle={Plastic cuckoo filters: growing a filter}]{hyperref}
...
An element consists of two groups of bits: one of size 8, and the other of size up to 7.
...
Element := {fingerprint: `$\ints_2^8$`, tail: `$\cup_{i \le 7} \ints_2^i$`}
Slot := Element`$_\bot$`
Bucket := Slot[4]
...
InsertSide(hashed: `$\ints_2^{k+8}$`, tail: `$\cup_{i \le 7} \ints_2^i$`,
  ...
  bucket := side.Bucket[permuted[0, k)]
  new_element = InsertBucket(permuted[k, k+8), tail, bucket)
...
In a minimal plastic cuckoo filter (``MPCF''), each element has a fingerprint of size 7 or 8 and a tail of size up to 6.
A bucket consists of four (possibly empty) slots, each of which can hold one element.
...
\begin{figure}
\begin{lstlisting}[escapeinside={`}{`}]
MElement := {fingerprint: `$\ints_2^7 \cup \ints_2^8$`, tail: `$\cup_{i \le 6} \ints_2^i$`}
...
If there are larger and smaller levels, then every element in the larger levels has a fingerprint of size 7, not 8.
