...
These structures support two operations -- insert and lookup; lookup always returns true on elements inserted into the structure; it also returns true with some probability $0 < \varepsilon \ll 1$ on elements {\em not} inserted into the structure.
...
Upon a positive lookup in the filter, the run identifier is retrieved, and a more expensive probe of that run can begin.
...
%Section~\ref{hibp} discusses \taffy{} filters used 
...
\TCF{}s contribute a new understanding of cuckoo filters as dictionaries.
...
Maps of size $n$ with keys from a universe of size $U$ can be na\"ively stored in $n \lg U$ bits by storing every element (in any order) in an array of size $n$.
...
Backyard cuckoo hashing handles this by using invertible hash functions~\cite{backyard}.
...
The extra bit in the key is acquired by ``stealing'' a single bit from the tail.
...
Split block Bloom filters have proven to be the fastest dynamic Bloom filters for doing single-element lookups in recent works on the matter~\cite{overtakes,ribbon,bloom-simd}.
...
\Taffy{} block filters' lookup operation requires $\lg n$ lookup operations on their sub-filters.
...
The former is called the {\em fingerprint} and is tested for equality when executing the lookup operation; the latter is called the {\em tail} and is the unused parts of the hashed key that will eventually be used in the fingerprint (after permuting -- see below).
...
THe probability that lookup will return true on a randomly-chosen key is no more than $8\cdot2^{-F}$ and is in practice a good bit lower; see Figure~\ref{ideal-bits-per-item}.
...
Applying this to \TCF{}s, since the space usage is linear, and since the ``sides'' (the two arrays of buckets) are of length $\lg n - O(1)$, then by reversing the quotienting operation we get that the probability that any random value that {\em wasn't} inserted matches with any of the existing elements is $O(n) 2^{-\lg n + O(1) - F} = 2^{-F+O(1)}$.
...
Freezing the \TCF{} produces a filter with a space usage of only 2.5GiB.
The omission of the tails leaves the false positive rate higher, at 0.73\%, but the lookup faster, at 74 nanoseconds.
