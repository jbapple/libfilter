...
A single growable filter structure can reduce this to a single filter query by storing one structure for all keys, rather than $\lg n$ structures.\footnote{The ``Chucky'' system is built on this premise, but requires a full filter rewrite at each last-level compaction~\cite{chucky}.}
