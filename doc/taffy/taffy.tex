...
A {\em side} consists of $2^k$ buckets for some $k$ as well as a random permutation on $\ints_2^{k+F}$. %\footnote{$\ints_2^m$ and $\ints_{2^m}$.}
...
\begin{enumerate}
\item Applies a permutation to the most-significant $k + F$ bits in the key. %(Listing~\ref{tcf-lookup}, line 14)
...
Instead, two candidate keys are created from the reverse-permuted $k+F$ bits by appending a zero and a one. %(Listing~\ref{tcf-upsize}, line 5)
