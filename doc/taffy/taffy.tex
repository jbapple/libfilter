...
After adding $n$ elements to the filter, the filter holds $n/2$ fingerprint-tail pairs with tail length $\lg \lg N$, $n/4$ pairs with tail length $\lg \lg N - 1$, and \dots $n/\lg N$ pairs with tail length 1.
...
at each point, the structure has a size which is a power of two, sometimes allocating twice as mush space as is needed. (See Figure~\ref{space-steps}.)
