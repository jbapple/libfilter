...
Pagh et al. ends with an open problem of implementing these structures in a practical way.~\cite{psw}
...
See Section~\ref{eval} for performance of TBFs compared to pre-size split block BLoom filters.
...
In each chart, all taffy filters are configured with an initial capacity of 1. %, however the MTCF has a minimum size of 2 bytes/entry $\cdot$ 32 levels $\cdot$ 2 sides/level $\cdot$ 1 entry / side = 128 bytes.
...
For comparison, the graphs also include a cuckoo filter (labeled ``CF'') with fingerprints of size 12 and a split block Bloom filter (labeled ``SBBF'') sized to hold 100 million elements with an fpp of 0.1\%.
...
\item Are union, intersection, freeze, or thaw operations needed?
