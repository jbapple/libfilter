...
\begin{enumerate}
\item Section~\ref{pbf} presents {\em taffy block filters} ``TBFs'', a practical implementation of the ``construction 1'' AMQ structure from Pagh et al.~\cite{psw}.
\item Section~\ref{tcf} presents {\em taffy cuckoo filters} (``TCFs''), a practical implementation of the ``construction 2'' from Pagh et al.
...
\begin{figure}
\includegraphics[width=\textwidth]{pagh-1-diagram}
...
This work extends the work of Pagh et al. to support Bloom filters in construction 1 and space-efficient cuckoo hashing in construction two.
This work also implements the Pagh et al. structures for the first time and reccoments circumstances under which each construction should be used.
...
The oldest example of this are from Knuth; the technique is generally called quotienting.~\cite{knuth,quotient-filter}
Taffy cuckoo filters are based on quotienting in cuckoo hash tables, as used in~\cite{backyard}.
...
Instead, Maier et al. uses the cuckoo hashing kick operation to incrementally resize a hash table.~\cite{dysect}
...
The ``construction 1'' structure from Pagh et al. consists of a set of filters of decreasing false positive probabilities but exponentially increasing size (in terms of the number of distinct elements).~\cite{psw}
...
\[
\sum_{i<\lg n} \lg (i^2 \pi^2 /(6 \varepsilon)) \\
= \sum_{i<\lg n} \lg (i^2) + \lg( \pi^2) - \lg 6 + \lg (1/\varepsilon) 
= \Theta(\lg^2 n + \lg n \lg (1/\varepsilon))
\]

In order to keep the query time down to $\lg n$, Pagh et al use dictionary-based filters such as cuckoo filters or Raman and Rao's dictionary, rather than Bloom filters, are used.~\cite{succinct,psw}
...
\begin{figure}
  \includegraphics[width=8cm]{sbbf-diagram}
...
%%  LocalWords:  Wieder longhead TBF onn Raman Rao's Rao Maier
%%  LocalWords:  Vasallo Breslow
