...
TODO: cite original cuckoo table paper in addition to the cuckoo filter table
...
TODO: cite original Bloom filter paper

A standard Bloom filter sets $\lg (1/\varepsilon)$ bits in an array of size $m$, holding $m \ln 2 / \lg(1/\varepsilon)$ distinct elements.
...
When a block Bloom filter is used with block size $B = 256$, $k = 8$ hash functions, and lane size $L = 32$~, it is possible to use SIMD instructions to perform eight hash function computations at once using single-instruction-multiple-data instructions, i.e. vectorization.
%in what Polychroniou and Ross call ``horizontal vectorization''.~\cite{horizontal}
The resulting Bloom filter has constant-time branch-free insert and lookup and is consistently faster than a cuckoo filter of the same size (See Figure~\ref{lookup-both}).~\cite{cuckoo-filter-github,ultra-fast,overtakes}
...
\begin{enumerate}
  ...
  \item Because fingerprints have variable size, there must be two permutations per side, one for each size of fingerprint.
  \item Because there are two permutations per side, a key may be in one of up to four distinct buckets (two per side), which decreases the performance and increases the false positive probability.
\end{enumerate}
...
TBFs, TCFs, and MTCFs have been implemented and tested for correctness; this section describes their space usage, false positive probabilities, and performance under the insert and upsize operation.
...
For comparison, the graphs also include a cuckoo filter (labelled ``CF'') and a split block Bloom filter (labelled ``SBBF''), each sized to hold 100 million elements with an fpp of 0.1\%.
...
When a filter doubles in size, the number of bits per element doubles as well.
MTCFs smooth out this pattern, but do not eliminate it.
...
Figures~\ref{insert-time},~\ref{arm-insert-time},~\ref{lookup-both},~and~\ref{arm-lookup-both} show the performance of the taffy filters.
For inserts, TBFs are the fastest of the five variants.
It is faster than the other cuckoo filters because is it simple, branch-free, and induces a single cache miss.
It is faster than the pre-size split block Bloom filter because, while being built, the entirety of the filter fits in cache until about 10 million elements have been inserted.
This holds true across both x86 and ARM machines

For lookups, the situation is more complex.
Here, the split block Bloom filter is mostly fastest.
Of the resizable filters, the taffy cuckoo filter is the astest once the size of the filter is over 4000 elements, in the ARM case, or one million elements, in the x86 case.
One reason for this is that the ARM split block Bloom filters that the TBF is based on nly have 128-bit vector units, not 256-bit units like the x86 machine does.
...
Otherwise, the practitioner must ask themselves:
