...
\newcommand{\spetwo}[1]{{\color{blue}{#1}}}
...
\spetwo{These structures represent sets approximately, and support at least two operations -- insert and lookup; lookup always returns true on elements in the structure; it also returns true with some probability $\varepsilon$ on elements {\em not} in the structure.}
...
\spetwo{However, unlike hash tables, cuckoo filters and Bloom filters must be initialized with the intended number of maximum entries, and cannot grow larger --}
...
\spetwo{Bloom filters support the operations insert -- which adds an item to the set -- and lookup, which returns true if an element is in the filter; if an element is not in the filter, true is returned with some configurable probability $\varepsilon$.}
...
\hline $\omega(1)$ lookup & dynamic \spetwo{Bloom} \cite{dynamic-bloom}, scalable \spetwo{Bloom} \cite{scalable-bloom}, dynamic cuckoo \cite{dynamic-cuckoo}, monkey \cite{monkey}\\
...
\hline $B$ & The size of a block in a \spetwo{blocked} Bloom filter. \\
...
See Section~\ref{tcf}. \\
\hline \spetwo{$A \cup B$} & \spetwo{The union of sets $A$ and $B$. } \\
\hline
\end{tabular}
...
\spetwo{Blocked} Bloom filters reduce the number of memory accesses to 1 at a cost of a slightly increased false positive probability.\cite{block-bloom}

Each \spetwo{blocked} Bloom filter is implemented as an array of non-overlapping blocks; see Figure~\ref{sbbf-diagram}.
Each block is itself a Bloom filter.
Blocks are no larger than a single cache line.
To insert a key, the key is hashed to select the block to use, mapping a key $x$ to $h(x) \bmod m/B$, where $h$ is the hash function, $m$ is the size of the \spetwo{blocked} Bloom filter and $B$ is the size of each block.

In split block Bloom filters, once a block is selected, it is used as a ``split'' Bloom filter.\cite{split-bloom}
In a standard Bloom filter, to insert a key $x$, $k = m \ln 2 / n$ hash functions are applied to $x$, and each bit $h_i(x) \bmod \spetwo{m}$ is set, $0 \le i < k$.
...
When a \spetwo{split} block Bloom filter is used with block size $B = 256$ and lane size $L = 32$, it is possible to use SIMD instructions to perform the eight hash function computations at once, set the eight bits at once (one per 32-bit lane), or check those eight bits at once.
...
In Listing~\ref{tcf-types},       \spetwo{ $A \cup B$ represents a union of $A$ and $B$.}
...
Element := {fingerprint: `$\ints_2^F$`, tail: `$\cup_{i \le T} \ints_2^i$`}
Slot := Element`$_\bot$`
Bucket := Slot[`$b$`]
...
Element := {fingerprint: `$\ints_2^F$`, tail: `\spetwo{$\cup_{i \le T} \ints_2^i$}`}
Slot := Element`$_\bot$`
Bucket := Slot[`$b$`]
Side(a) := {Bucket[2`$^a$`], Permutation: `$S_{2^{k+F}}$`}
...
All bits following \spetwo{the first} one bit are the tail.
Bits $1-F$ are the $F$-bit fingerprint.
\spetwo{For example, the last $T+1$ bits {\tt 010000} represent a tail of size 4, {\tt 0000}, while {\tt 000001} represent a tail of length zero.
If the last $T+1$ bits are all 0's, then the slot is empty, which is distinct from an element with a tail of length zero.}
...
\spetwo{If a fingerprint/tail combination is already present, insert terminates, rather than inserting a value twice.}
...
\spetwo{
\paragraph{Delete}
Unlike in cuckoo filters, each inserted value may correspond to many slots in the structure.
In particular, after $n$ insertions, the first inserted key corresponds to $2^{\lceil\lg n\rceil - T}$ slots.
This makes deletion a challenge.
Pagh \etalia{} describe how to extend the structure to allow deletions.\cite{psw}
It requires an additional structure and also an additional field in each slot indicating the original number of bits in the quotient when the value was inserted.
This adds an additional $\lg \lg N$ bits of storage to each slot.
Because this would increase the size of the structure substantially in practice, deletion is thus not supported in \TCF{}s.
}
...
Element := {fingerprint: `$\ints_2^{F-1} \cup \ints_2^F$`, tail: `$\cup_{i \le T} \ints_2^i$`}
Slot := Element`$_\bot$`
Bucket := Slot[`$b$`]
Level(a) := Bucket[2][2`$^a$`] `$\cup$` Bucket[2][2`$^{a+1}$`]
Permutation(a) := `$S_{2^{p + a + F - 1}} \times S_{2^{p + a + F}}$`
...
Element := {fingerprint: `$\ints_2^{F-1} \spetwo{\cup} \ints_2^F$`, tail: `$\spetwo{\cup}_{i \le T} \ints_2^i$`}
Slot := Element`$_\bot$`
Bucket := Slot[`$b$`]
Level(a) := Bucket[2][2`$^a$`] `$\spetwo{\cup}$` Bucket[2][2`$^{a+1}$`]
...
\TBF{}s, \TCF{}s, and \MTCF{}s have been implemented \spetwo{in C++}; this section describes their space usage, false positive probabilities, and performance.% under the insert and lookup operations.
\ifanon
\else
\spetwo{The code for \taffy{} filters is available on GitHub under a permissive open-source license.\ifanon\else\footnote{\url{https://github.com/jbapple/libfilter}}\fi}
\fi
...
Figures~\ref{insert-time},~\ref{arm-insert-time},~\ref{lookup-both},~and~\ref{arm-lookup-both} show the performance of \taffy{} filter operations.\footnote{All charts of time show the minimum over nine runs. \spetwo{See Figure~\ref{median-deviation} for the lookup benchmark deviation (median over minimum difference over the nine runs).}}
...
\centering
  ...
  \includegraphics[width=3.333in]{lookup-median-overage}
  \caption{
    \label{median-deviation}
    \spetwo{For lookup times, the median measurement of lookup time is slower than the minimum lookup time by the percentage on the y axis.}
    % This is for keys that are not found; the corresponding figure for found keys looks nearly identical.
  }
...
\spetwo{
The use case is of incermentally processing the new versions and new user passwords without reconstituting the filter from scratch.
The password checking service needing to use the database can centrally store, rather than the full data set, just a taffy filter containing a small-space approximation of the set.
New passwords can be checked against the database before allowing users to set a password that has already been leaked.
The taffy filters are small enough to store in main memory, so queries need not go to disk for the full set of password hashes in the dataset.
}
...
For the ``Raw, sorted'' column, we consider the cost of storing 64 bits of each hash in a single array with sorting as the input method and \spetwo{\texttt{std::binary\_search}} as the lookup method.\footnote{\spetwo{Interpolation search could reduce the number of probes required by a factor of $\theta(\log n / \log \log n)$ at the cost of an additional multiplication, addition, and division per probe.}}
